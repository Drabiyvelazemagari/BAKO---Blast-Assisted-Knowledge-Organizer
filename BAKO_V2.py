...
def is_accession_like(s: str) -> bool:
    ...
    if low.startswith(("node_", "contig", "scaffold", "unitig")):
        return False
    # Assembly accessions are not nuccore records; skip them to avoid guaranteed misses.
    if s.startswith(("GCA_", "GCF_")):
        return False
    # GenBank/RefSeq nucleotide accessions (rough but inclusive of RefSeq prefixes).
    patterns = [
        r"^[A-Z]{1,4}\d{5,}(?:\.\d+)?$",          # e.g., CP000001.1, AE000511.1
        r"^[A-Z]{2}_\d{5,}(?:\.\d+)?$",           # e.g., NC_000913.3
        r"^NZ_[A-Z]{2}_\d{5,}(?:\.\d+)?$",        # e.g., NZ_CP000001.1
        r"^[A-Z]{4}\d{8}(?:\.\d+)?$",             # WGS-style (e.g., ABCD01000001.1)
    ]
